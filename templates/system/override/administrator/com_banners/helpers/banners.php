...
class BannersHelper
{
	...
	public static function addSubmenu($vName)
	{
		...
		//new submenu positions
		JSubMenuHelper::addEntry(
			JText::_('COM_BANNERS_SUBMENU_POSITIONS'),
			'index.php?option=com_banners&view=positions',
			$vName == 'positions'
		);
	...
	public static function getPositionOptions()
	{
		...
		$query->select('id As value, name As text');
		$query->from('#__banner_positions AS a');
		$query->order('a.name');
		...
		array_unshift($options, JHtml::_('select.option', '0', JText::_('COM_BANNERS_NO_POSITION')));
